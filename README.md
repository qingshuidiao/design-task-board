...
完成后，网页会自动读取 `design_tasks` 这张表。当前安全默认值是：匿名访问只能读取任务，不能新增、编辑、删除或拖拽同步任务。

不要把编辑口令或编辑权限校验放在前端页面里。前端只能做界面显示控制，不能承担真正的权限保护；如果要让指定成员可编辑，需要接 Supabase Auth、企业登录，或通过后端接口在服务端校验身份后再写入数据。
